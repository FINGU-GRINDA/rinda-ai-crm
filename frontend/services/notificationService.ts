import { Notification, Customer, ScheduledFollowUp, CalendarEvent } from '../types';
import { getDueFollowUps, getUpcomingFollowUps, getOverdueFollowUps } from './autoFollowUpService';
...
export const createFollowUpDueNotification = (followUp: ScheduledFollowUp, customer: Customer): Notification => {
  return {
...
export const createLostDealNotification = (customer: Customer, daysSinceLost: number): Notification => {
  return {
...
export const checkFollowUpNotifications = (customers: Customer[]): Notification[] => {
  ...
  for (const followUp of dueFollowUps) {
    ...
    if (customer) {
      ...
      const followUpTypeText = followUp.type === 'email' ? '이메일' :
                               followUp.type === 'call' ? '전화' :
                               followUp.type === 'meeting' ? '미팅' : '메시지';
...
export const checkAndSendDailyDigest = async (customers: Customer[]): Promise<void> => {
  ...
  await sendDailyDigestNotification(pendingCount, overdueFollowUps.length, todayFollowUps).catch(err => {
