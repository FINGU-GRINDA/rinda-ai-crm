...
export const suggestFollowUpTiming = (
  ...
  const _daysSinceLost = Math.floor((now - lostAtTimestamp) / (1000 * 60 * 60 * 24))
  ...
  if (strategy.recommendedTiming.includes("즉시") || strategy.recommendedTiming.includes("지금")) {
    ...
    strategy.recommendedTiming.includes("1주") ||
    strategy.recommendedTiming.includes("7일")
  ) {
    suggestedDays = 7
  } else if (
    strategy.recommendedTiming.includes("2주") ||
    strategy.recommendedTiming.includes("14일")
  ) {
    suggestedDays = 14
  } else if (
    strategy.recommendedTiming.includes("30일") ||
    strategy.recommendedTiming.includes("한 달")
  ) {
    suggestedDays = 30
