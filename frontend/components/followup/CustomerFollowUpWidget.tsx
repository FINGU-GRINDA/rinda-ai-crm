...
export const CustomerFollowUpWidget: React.FC<CustomerFollowUpWidgetProps> = ({
  ...
  const handleFormSubmit = (_followUp: ScheduledFollowUp) => {
    loadFollowUps()
    onFollowUpChange?.()
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
      ...
      <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        ...
        >
          <IconPlus className="w-4 h-4" />새 스케줄
        </button>
      </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100">
            ...
            <div className="space-y-2">
              {completedFollowUps.map((followUp) => (
                <div key={followUp.id} className="text-xs text-slate-500 flex items-center gap-2">
                  {getTypeIcon(followUp.type)}
                  <span className="flex-1 truncate">{followUp.reason}</span>
                  <span className="text-slate-400">
                    {new Date(followUp.completedAt || followUp.createdAt).toLocaleDateString(
                      "ko-KR",
