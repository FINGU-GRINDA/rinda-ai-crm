...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    ...
    >
      {icon && (
        <div
          className={`flex items-center justify-center rounded-full bg-slate-100 text-slate-500 mb-3 ${s.icon}`}
        >
          {icon}
        </div>
      )}
      <h4 className={`font-semibold text-slate-700 mb-1 ${s.title}`}>{title}</h4>
      {description && <p className={`text-slate-500 leading-relaxed ${s.desc}`}>{description}</p>}
