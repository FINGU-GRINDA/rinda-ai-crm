...
import { AddCustomerModal, DeleteConfirmModal, DismissProspectModal } from "./components/modals"
...
const ProposalGenerator = lazy(() =>
  import("./components/ProposalGenerator").then((m) => ({ default: m.ProposalGenerator })),
)
const ICPSettings = lazy(() =>
  import("./components/ICPSettings").then((m) => ({ default: m.ICPSettings })),
)
const AutoFollowUpScheduler = lazy(() =>
  import("./components/AutoFollowUpScheduler").then((m) => ({ default: m.AutoFollowUpScheduler })),
)
const MeetingPrep = lazy(() =>
  import("./components/MeetingPrep").then((m) => ({ default: m.MeetingPrep })),
)
...
export const AppDashboard: React.FC = () => {
  ...
  const [_customersError, setCustomersError] = useState<string | null>(null)
  ...
  const [_collectionSettings, setCollectionSettings] = useState(() => getCollectionSettings())

  // Phase 1: New Features States
  const [_contextualSuggestions, setContextualSuggestions] = useState<ContextualSuggestion[]>([])
  ...
  }, [customers, fetchProspectsFromBackend])
    ...
    try {
      ...
      const enrichedData: EnrichedData = {
        ...
        recentNews: enrichmentData.recentNews
          ? typeof enrichmentData.recentNews === "string"
            ? [enrichmentData.recentNews]
            : enrichmentData.recentNews
          : [],
        competitors: enrichmentData.competitors
          ? typeof enrichmentData.competitors === "string"
            ? JSON.parse(enrichmentData.competitors)
            : enrichmentData.competitors
          : [],
        salesOpportunity: enrichmentData.salesOpportunity || "",
        sources: enrichmentData.sources
          ? typeof enrichmentData.sources === "string"
            ? JSON.parse(enrichmentData.sources)
  ...
  const handleSaveProposal = useCallback(
    async (proposalData: { title: string; content: string; imageUrl?: string }) => {
  ...
  const handleBusinessCardScanComplete = useCallback(
    async (_data: BusinessCardData, customerId?: string) => {
      if (customerId) {
  ...
  const handleMeetingRecordComplete = useCallback(
    async (summary: MeetingSummary) => {
      await fetchCustomers()
          ...
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-amber-800">
              ...
              <span className="text-sm font-medium">
                백엔드 서버에 연결할 수 없습니다. 데이터가 저장되지 않을 수 있습니다.
              </span>
            </div>
            <button
              onClick={async () => {
          ...
          {activeTab === "prospects" ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <p className="text-sm text-slate-500 mb-4">
                다른 키워드로 검색해보시거나, 새로운 고객을 추가해보세요.
              </p>
              <button
