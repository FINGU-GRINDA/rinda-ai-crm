...
import { IconX, IconCheck, IconSparkles, IconClock } from '../../Icons';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">ICP 프로필을 설정하고 AI 기반 잠재고객 수집을 관리합니다.</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">설정한 주기마다 자동으로 잠재 고객을 수집합니다</p>
            </div>
      ...
      <div className="border-t border-slate-200 pt-6">
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {formData.industries?.map((industry, index) => (
                <span key={index} className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm">
                  {industry}
                  <button onClick={() => removeIndustry(index)} className="text-blue-700 hover:text-blue-900">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">키워드 * (쉼표로 구분)</label>
            <div className="flex gap-2 mb-2">
              <input
                type="text"
                value={keywordInput}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">회사 규모 (선택)</label>
            <input
              type="text"
          ...
          <div className="flex gap-3">
            ...
            {editingId && (
              <button onClick={resetForm} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200">
                취소
              </button>
            )}
          </div>
